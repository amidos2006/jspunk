...
/// @param smoothing make sure it is pixel perfect or not
/// @param menu set this to true to enable context menu
function allegro_init_all(id,w,h,smoothing,menu,enable_keys)
{
	install_allegro();
	set_gfx_mode(id,w,h,smoothing);
	install_mouse(menu);
...
{
	log("Loading json " + filename + "...");
	var jsonFile = {data:null, ready: false, type: "json"};
	_downloadables.push(jsonFile);
	var xobj = new XMLHttpRequest();
	xobj.overrideMimeType("application/json");
	xobj.open('GET', filename, true);
	xobj.onreadystatechange = function () {
		if (xobj.readyState == 4 && xobj.status == "200") {
			log("JSON " + filename + " loaded, #chars: " + xobj.responseText.length + "!");
...
function load_xml(filename) {
	log("Loading xml " + filename + "...");
	var xmlFile = { data: null, ready: false, type: "xml" };
	_downloadables.push(xmlFile);
	var xobj = new XMLHttpRequest();
	xobj.overrideMimeType("application/xml");
	xobj.open('GET', filename, true);
	xobj.onreadystatechange = function () {
		if (xobj.readyState == 4 && xobj.status == "200") {
			log("XML " + filename + " loaded, #chars: " + xobj.responseText.length + "!");
...
function load_txt(filename) {
	log("Loading txt " + filename + "...");
	var txtFile = { data: null, ready: false, type: "txt" };
	_downloadables.push(txtFile);
	var xobj = new XMLHttpRequest();
	xobj.overrideMimeType("application/txt");
	xobj.open('GET', filename, true);
	xobj.onreadystatechange = function () {
		if (xobj.readyState == 4 && xobj.status == "200") {
			log("TXT " + filename + " loaded, #chars: " + xobj.responseText.length + "!");
...
function load_bitmap_font(filename){
	log("Loading font " + filename + "...");
	var fontFile = { bitmap: null, data:null, ready: false, type: "bmpfnt" };
	_downloadables.push(fontFile);
	var xobj = new XMLHttpRequest();
	xobj.overrideMimeType("application/xml");
	xobj.open('GET', filename, true);
	xobj.onreadystatechange = function () {
		if (xobj.readyState == 4 && xobj.status == "200") {
			log("FONT " + filename + " loaded, #chars: " + xobj.responseText.length + "!");
			let xmlFile = xobj.responseXML;
			fontFile.bitmap = load_bitmap(filename.substring(0, filename.lastIndexOf("/")) + 
...
/// @param pixel perfect or not
/// @return 0 on success or -1 on error
function set_gfx_mode(canvas_id,width,height,smoothing)
{
...
/// Draws a rectangle from the source image with rotation
/// @param bmp target bitmap
/// @param sprite sprite bitmap
/// @param x,y coordinates of where to draw the image
/// @param cx,cy coordinates of the center of the image
/// @param angle angle of rotation in degrees
/// @param sx,sy scale for horizontal and vertical
/// @param wx,wy location of the window in the source input
/// @param ww,wh the window width and window height
function pivot_window_sprite(bmp,sprite,x,y,cx,cy,angle,sx,sy,wx,wy,ww,wh){
	bmp.context.save();
	bmp.context.translate(x, y);
	bmp.context.rotate(RAD(angle));
	bmp.context.scale(sx, sy);
	bmp.context.translate(-cx, -cy);
	bmp.context.drawImage(sprite.canvas,wx,wy,ww,wh,0,0,ww,wh);
	bmp.context.restore();
}
