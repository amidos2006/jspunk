...
import { MouseKeys } from "./input.js";
...
export class LoadingWorld extends World {
    constructor(loadFunction, doneFunction, drawFunction, pressFunction) {
        if(pressFunction == undefined){
            pressFunction = function(){
                return JSP.input.mousePressed(MouseKeys.LEFT);
            }
        }
        super();
    ...
    draw(){
        ...
        if(!drawn){
            ...
            if(JSP.loader.getLoadingProgress() >= 1){
                JSP.renderTarget.drawText(JSP.loader.getFile("font"), "--CLICK TO START--", JSP.renderTarget.width/2 - 40, 
        ...
        if(JSP.loader.getLoadingProgress() >= 1){
            if(!this._pressFunction || this._pressFunction()){
